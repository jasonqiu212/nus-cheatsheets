...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
\includegraphics[scale=0.106]{quadric-surfaces.jpg}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item If $f$ and $g$ are cont., then $f \pm g$, $fg$, $\frac{f}{g}$, $f \circ g$ are cont.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $f_x(x,y) = \lim_{h \rightarrow 0} \frac{f(x+h,y)-f(x,y)}{h}$
...
\begin{itemize}
    \item Type I: If $E = \{(x,y,z):(x,y \in D, u_1(x,y) \leq z \leq u_2(x,y))\}$ where $D$ is projection of $E$ onto $xy$-plane, then $\iiint_E f dV = \iint_D (\int_{u_1(x,y)}^{u_2(x,y)} f dz)dA$
    \item Type II: If $E = \{(x,y,z):(y,z \in D, u_1(y,z) \leq x \leq u_2(y,z))\}$ where $D$ is projection of $E$ onto $yz$-plane, then $\iiint_E f dV = \iint_D (\int_{u_1(y,z)}^{u_2(y,z)} f dx)dA$
    \item Type III: If $E = \{(x,y,z):(x,z \in D, u_1(x,z) \leq y \leq u_2(x,z))\}$ where $D$ is projection of $E$ onto $xz$-plane, then $\iiint_E f dV = \iint_D (\int_{u_1(x,z)}^{u_2(x,z)} f dy)dA$
