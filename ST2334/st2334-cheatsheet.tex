...
\begin{itemize}
    ...
    \item If both $X$ and $Y$ are discrete/continuous, then $(X, Y)$ is discrete/continuous.
\end{itemize}
...
\begin{itemize}
    ...
    \item Let $X \sim N(\mu, \sigma^2)$. We can compute $P(x_1 < X < x_2)$ by standardization:
...
\begin{itemize}
    \item \keyword{Null Hypothesis}{$H_0$ Statement that parameter takes some value}
    \item \keyword{Alternative Hypothesis}{$H_1$ Statement that parameter falls in alt. range}
    \item \keyword{2-Sided Test}{If $H_1$ is "Parameter is $\neq$ to value under $H_0$"}
    \item \keyword{Right-Sided Test}{If $H_1$ is "Parameter is $>$ to value under $H_0$"}
    \item \keyword{Left-Sided Test}{If $H_1$ is "Parameter is $<$ to value under $H_0$"}
\end{itemize}
...
\begin{itemize}
    \item \keyword{Level of Significance}{$\alpha$ Probability of rejecting $H_0$ when it is true. i.e.}
    \[\alpha = P(\text{Type I error})\]
    \item \keyword{Power of the Test}{$1 - \beta = P(\text{Reject $H_0$} | \text{$H_0$ is false})$ where}
    \[\beta = P(\text{Type II error})\]
\end{itemize}

\subsubsection{Step 3: Test Statistic, Distribution, and Rejection Region}
\begin{itemize}
    \item \keyword{Test Statistic}{Statistic used to see how far away from $H_0$ the data is}
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \keyword{p-Value}{Conditional probability that test statistic is as extreme as observed value, given $H_0$ is true}
        \item $H_1: \mu \neq \mu_0$: $p = 2P(Z < -|z|)$
